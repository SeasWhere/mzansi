...
if __name__ == "__main__":
    try:
        ...
        st.write("Mzansi app is running. Visit [http://localhost:5000](http://localhost:5000) to access the application.")
    except ImportError:
        # If streamlit is not available, run with production-grade server if possible.
        if serve:
            serve(app, host="0.0.0.0", port=5000, threads=2)
        else:
            app.run(host="0.0.0.0", port=5000, debug=True)
