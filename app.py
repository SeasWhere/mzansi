...
# Revised main block to serve a full Streamlit GUI
if __name__ == "__main__":
    try:
        import streamlit as st

        st.title("Mzansi Filing Viewer")

        cik = st.text_input("Company CIK (numbers only)")
        ticker = st.text_input("Ticker (optional)")
        months = [(str(i), datetime(2000, i, 1).strftime('%B')) for i in range(1, 13)]
        month_dict = {num: month for num, month in months}
        fy_month = st.selectbox("Fiscal Year-End Month", options=[num for num, _ in months],
                                format_func=lambda x: month_dict[x])
        fy_adjust = st.selectbox("Fiscal Year Basis", options=["Same Year", "Previous Year"])
        cleanup_flag = st.checkbox("Delete HTML and assets after PDF conversion")
        
        if st.button("Fetch Filing"):
            if not cik:
                st.error("Please enter a valid CIK.")
            else:
                log_lines = []
                with tempfile.TemporaryDirectory() as tmp_dir:
                    pdf_files = process_filing(cik, ticker, fy_month, fy_adjust, cleanup_flag, log_lines, tmp_dir)
                    if not any(pdf_files.values()):
                        st.error("No valid filings found for this criteria")
                    else:
                        zip_path = create_zip_archive(pdf_files, cik.zfill(10), log_lines, tmp_dir)
                        if not zip_path:
                            ...
                            with file_storage_lock:
                                app.config['FILE_STORAGE'][filename] = zip_data
                            st.success("Filings processed successfully")
                            st.download_button("Download ZIP Archive", zip_data, file_name=filename, mime="application/zip")
                st.text_area("Log", "\n".join(log_lines), height=300)
    except ImportError:
        # Fallback: run Flask normally if Streamlit is not available.
