...
# Try to import Waitress; if unavailable, set serve to None.
try:
...
def share_files():
    with file_storage_lock:
        files = [
            {"filename": k, "url": url_for("download_file", filename=k, _external=True)}
...
# For running on Streamlit Cloud. The code below launches the Flask app in a background thread.
if __name__ == "__main__":
    import streamlit as st

    def run_flask():
        # Use Waitress if available; otherwise fallback to Flask’s built-in server.
        if serve:
    ...
    # Display a simple message in Streamlit
    st.title("Mzansi EDGAR Fetcher")
    st.write("""
    Mzansi app is running in the background. 
    Because Streamlit Cloud doesn't allow direct access to port 5000, 
    this interface might appear blank. 
    """)
    st.write("""
    If you are hosting somewhere that exposes port 5000, 
    visit [http://localhost:5000](http://localhost:5000) (or the equivalent on your host)
    to access the Flask interface.
    """)
