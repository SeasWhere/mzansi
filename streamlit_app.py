    ...
    try:
        ...
        for i in range(list_len):
            ...
            try:
                ...
                # Filter 2: Year Cutoff (Stop searching if year is before target)
                if 0 <= year_suffix < EARLIEST_FISCAL_YEAR_SUFFIX:
                    log_lines.append(f"Reached {period}. Stopping search for older filings.")
                    break # Exit loop once we hit filings older than target year
