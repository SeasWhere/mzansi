...
def fetch_json_with_retries(url, max_retries=3, delay=10):
    ...
    in case of SEC API limitations (e.g., 404 or other HTTP errors).
    """
    attempt = 0
    while attempt < max_retries:
        try:
            ...
            # Check for rate-limit or API limitation errors:
            err_msg = str(e)
...
def convert_to_pdf(html_path, form, date, accession, cik, ticker, fy_month_idx, fy_adjust, log_lines):
    """
    Uses fpdf to convert HTML content (extracted as text) to a simple PDF.
    """
    try:
