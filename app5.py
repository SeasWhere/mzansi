        ...
        <script>
            async function handleSubmit() {
                ...
                if (!('showDirectoryPicker' in window)) {
                    logDiv.innerHTML += '\\nError: Your browser does not support the Directory Picker API.';
                    return;
                }
...
# -----------------------------
# Revised main block for Streamlit
# -----------------------------
if __name__ == "__main__":
    try:
        import streamlit as st

        # Build a Streamlit UI for the app
        ...
        if st.button("Fetch Filing"):
            if not cik.isdigit():
                st.error("Please enter a numeric CIK.")
            else:
                log_lines = []
                with tempfile.TemporaryDirectory() as tmp_dir:
                    pdf_files = process_filing(cik, ticker, fy_month, fy_adjust, cleanup_flag, log_lines, tmp_dir)
                    if not any(pdf_files.values()):
                        ...
                        if not zip_path:
                            ...
                            with open(zip_path, 'rb') as f:
                                zip_data = f.read()
                            filename = os.path.basename(zip_path)
                            # Store in the Flask file storage in case needed
                            with file_storage_lock:
                                app.config['FILE_STORAGE'][filename] = zip_data
        ...
        # Fallback: run Flask if Streamlit is not available
